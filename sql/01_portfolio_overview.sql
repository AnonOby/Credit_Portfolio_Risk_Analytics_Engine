...
SELECT
    ...
    ROUND(SUM(funded_amnt)::numeric, 2)                     AS total_funded_amount,
    ROUND(AVG(funded_amnt)::numeric, 2)                     AS avg_funded_amount,
    ROUND(AVG(int_rate)::numeric, 2)                        AS avg_interest_rate,
    ROUND(MIN(int_rate)::numeric, 2)                        AS min_interest_rate,
    ROUND(MAX(int_rate)::numeric, 2)                        AS max_interest_rate,
...
SELECT
    grade,
    COUNT(*)                                AS total_loans,
    ROUND(COUNT(*) * 100.0 / SUM(COUNT(*)) OVER ()::numeric, 2) AS pct_of_portfolio,
    ROUND(SUM(loan_amnt)::numeric, 2)      AS total_loan_amount,
    ROUND(AVG(loan_amnt)::numeric, 2)      AS avg_loan_amount,
    ROUND(AVG(int_rate)::numeric, 2)       AS avg_interest_rate,
...
SELECT
    grade,
    ROUND(AVG(fico_range_low)::numeric, 1)             AS avg_fico_low,
    ROUND(AVG(fico_range_high)::numeric, 1)            AS avg_fico_high,
    ROUND(AVG((fico_range_low + fico_range_high) / 2.0)::numeric, 1) AS avg_fico_midpoint,
    ROUND(MIN(fico_range_low)::numeric, 1)             AS min_fico_low,
    ROUND(MAX(fico_range_high)::numeric, 1)            AS max_fico_high
FROM loans_master
GROUP BY grade
ORDER BY grade;